...
from keras_bert_ner_light.helper import deploy_args_parser
...
def run_deploy():
    ...
    app.model_configs["featurizer_url"] = args.featurizer_url
    app.model_configs["featurizer_port"] = args.featurizer_port
    app.model_configs["featurizer_timeout"] = args.featurizer_timeout
    app.model_configs["max_len"] = args.max_len
    return graph, sess
...
def parse(text):
    ...
    if isinstance(text, str):
        ...
        features = text
    features = get_model_inputs(app.model_configs["featurizer_url"], app.model_configs["featurizer_port"],
                                app.model_configs["featurizer_timeout"], features, app.model_configs["max_len"])
    # 解码
    # 显存不足时注释掉以下代码
    ...
    with graph.as_default():
        keras.backend.set_session(sess)
        decode_res = app.model_configs["viterbi_decoder"].decode(features)
