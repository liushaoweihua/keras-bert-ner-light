...
def train(args):
    ...
    # 数据准备
    processor = Processor(args.train_data, args.tag_padding)
    featurizer = Featurizer(args.featurizer_url, args.featurizer_port, args.featurizer_timeout)
    train_data, train_y = processor.process(args.train_data, args.max_len)
    train_x = featurizer.parse(train_data)
    ...
    model_save_path = os.path.abspath(
        os.path.join(args.save_path, "%s.h5" % (model_type)))
    log_save_path = os.path.abspath(
        os.path.join(args.save_path, "%s-%s.log" % (model_type, time.strftime("%Y%m%d_%H%M%S"))))
    if args.best_fit:
        best_fit_params = {
    ...
    model_configs = {
        ...
        "featurizer_url": args.featurizer_url,
        "featurizer_port": args.featurizer_port,
        "featurizer_timeout": args.featurizer_timeout}
